...
contract ETHTickets is ERC721Enumerable, Ownable {
    ...
    function contractURI() public pure returns (string memory) {
        ...
        bytes memory collectionJsonString = bytes(
            abi.encodePacked(
                '{"name":"ETH Atlantis",',
    ...
    modifier canMint() {
        ...
        require(
            vipTicketPrice == msg.value || gaTicketPrice == msg.value,
            "Ether value sent incorrect"
        );
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        string memory name = string(
            abi.encodePacked("Ticket #", id.toString())
        );
        string memory description = string(
            abi.encodePacked((vipTicketHolders[id] ? "VIP" : "GA"), " Access")
        );

        bytes memory tokenJsonString = bytes(
            abi.encodePacked(
                '{"name":"',
                name,
                '","description":"',
                ...
                (vipTicketHolders[id] ? "VIP" : "GA"),
                '"}],',
                '"owner":"',
                (uint160(ownerOf(id))).toHexString(20),
                '",',
                '"image":"',
                generateNftSvgByTokenId(id),
                '"}'
            )
        );
    ...
    function generateNftSvgByTokenId(
        uint256 id
    ...
    function renderNftSvgTopById(
        ...
        return
            string(
                abi.encodePacked(
                    '<linearGradient id="',
                    (vipTicketHolders[id] ? "vpBg" : "gaBg"),
                    '"><stop offset="5%" stop-color="#',
                    (vipTicketHolders[id] ? "460090" : "00385D"),
                    '" /><stop offset="95%" stop-color="#',
                    (vipTicketHolders[id] ? "61008C" : "00528B"),
                    '"/></linearGradient>',
                    '<g transform="matrix(0.966539,0,0,0.966539,4.93126,4.90586)"><path style="fill:url(#',
                    (vipTicketHolders[id] ? "vpBg" : "gaBg"),
                    ');" d="M0.071,201.848L300.111,201.848L300.111,10.272C300.111,4.656 295.552,0.097 289.936,0.097L10.245,0.097C4.63,0.097 0.071,4.656 0.071,10.272L0.071,201.848Z"/></g>',
                    '<g transform="matrix(1.6679,0,0,1.6679,-25,-40)"><text x="24px" y="54px" class="s1 s2b s3">ETH Atlantis</text></g>',
                    '<g transform="matrix(1.6679,0,0,1.6679,-25,-10)"><text x="24px" y="54px" class="s1 s2b s5">2024, July 4 / ',
                    (vipTicketHolders[id] ? "VIP" : "GA"),
                    "</text></g>",
                    '<g transform="matrix(1.6679,0,0,1.6679,-25,85)"><text x="24px" y="54px" class="s1 s2b s8">',
                    nftOwner,
                    "</text></g>"
                )
            );
    ...
    function renderNftSvgBottomById(
        uint256 id
    ) internal view returns (string memory) {
        ...
        return
            string(
                abi.encodePacked(
                    '<g transform="matrix(1,0,0,1,0,0.177859)"><path class="',
                    (vipTicketHolders[id] ? "s6" : "s7"),
                    '" d="M295,199.822L5,199.822L5,284.988C5,290.416 9.406,294.822 14.834,294.822L285.166,294.822C290.594,294.822 295,290.416 295,284.988L295,199.822Z"/></g>',
                    '<g transform="matrix(1.6679,0,0,1.6679,50,160)"><text x="20%" y="14%" style="dominant-baseline:central; text-anchor:middle;" class="s1 s2b s4">#',
                    id.toString(),
                    "</text></g>",
                    '<g transform="matrix(1.6679,0,0,1.6679,50,195)"><text x="20%" y="14%" style="dominant-baseline:central; text-anchor:middle;" class="s1 s2b s4">',
                    ticketType,
                    "</text></g>"
                )
            );
